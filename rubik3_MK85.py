...
This solves one side of 3*3*3 Rubik's cube: https://yurichev.com/blog/rubik2/
Way slower than solution using Z3, by factor of ~30.
...
def rotate(op, st, side, j):
	return s.If(op==0, rotate_FCW(st)[side][j],
		s.If(op==1, rotate_FCCW(st)[side][j],
		s.If(op==2, rotate_UCW(st)[side][j],
		s.If(op==3, rotate_UCCW(st)[side][j],
		s.If(op==4, rotate_DCW(st)[side][j],
		s.If(op==5, rotate_DCCW(st)[side][j],
		s.If(op==6, rotate_RCW(st)[side][j],
		s.If(op==7, rotate_RCCW(st)[side][j],
		s.If(op==8, rotate_LCW(st)[side][j],
		s.If(op==9, rotate_LCCW(st)[side][j],
		s.If(op==10, rotate_BCW(st)[side][j],
		s.If(op==11, rotate_BCCW(st)[side][j],
		s.If(op==12, rotate_FH(st)[side][j],
		s.If(op==13, rotate_UH(st)[side][j],
		s.If(op==14, rotate_DH(st)[side][j],
		s.If(op==15, rotate_RH(st)[side][j],
		s.If(op==16, rotate_LH(st)[side][j],
		s.If(op==17, rotate_BH(st)[side][j],
...
for STEPS in range(1, 20):
	...
	s=MK85()
	state=[[[s.Bool('state%d_%d_%d' % (n, side, i)) for i in range(FACELETS)] for side in range(FACES)] for n in range(STEPS+1)]

	op=[s.BitVec('op%d' % n, 5) for n in range(STEPS+1)]
	...
	if s.check():
		...
		#print m
		for n in range(STEPS):
			print move_names[m['op%d' % n]]
		exit(0)
"""
% time python rubik3_MK85.py
trying 1 steps
trying 2 steps
trying 3 steps
trying 4 steps
...
UCW
LCCW
python rubik3_MK85.py  2165.33s user 6.66s system 99% cpu 36:14.23 total
"""
